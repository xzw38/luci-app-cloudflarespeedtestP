...
return view.extend({
	...
	render: function(data) {
		...
		if (nodes.passwall && nodes.passwall.exists) {
			...
			o = s.taboption('passwalltab', form.Flag, 'passwall_sequential_ip',
				_('Fill Passwall nodes with top IPs in order'));
			o.description = _('When enabled, selected Passwall nodes are filled one by one with the top IPs from the latest speed test results');
			o.default = '0';
			o.rmempty = false;
		...
		if (nodes.passwall2 && nodes.passwall2.exists) {
			...
			o = s.taboption('passwall2tab', form.Flag, 'passwall2_sequential_ip',
				_('Fill Passwall2 nodes with top IPs in order'));
			o.description = _('When enabled, selected Passwall2 nodes are filled one by one with the top IPs from the latest speed test results');
			o.default = '0';
			o.rmempty = false;
