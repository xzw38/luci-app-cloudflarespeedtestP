...
本分支默认保留原版行为，只额外新增一个可选的 Passwall/Passwall2 顺序写入模式。

*   **按测速排名依次写入 Passwall 节点**：在第三方设置中启用后，会将最新测速结果中的前几个 IP 按顺序写入已选择的 Passwall 或 Passwall2 节点。例如第 1 个已选节点写入测速第 1 名 IP，第 2 个已选节点写入测速第 2 名 IP，以此类推。
*   **兼容原版行为**：该选项默认关闭。不开启时，插件仍然像原版一样，把单个最快 IP 写入所有已选择节点。
*   **自动匹配结果数量**：开启顺序写入后，测速会自动请求至少和已选 Passwall/Passwall2 节点数量相同的排名 IP。如果最终结果仍然不足，多出来的节点会跳过写入，不再重复填入第 1 名 IP。
